...
///
/// For objects which implement [`HasPosition`](struct.HasPosition.html) and [`HasHeight`](trait.HasHeight.html), the top coordinate is equal to `(self.position().y()`. In this case, `HasHeight` is a bound ensuring that the object can sensibly have a `self.top()`.
pub trait HasTop {
